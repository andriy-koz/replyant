...
| --- | --- | --- |
| Repeatability | High | Same 8 qualifying questions, same criteria, same data lookups |
| Judgment complexity | Low | Binary pass/fail against defined criteria |
| Data dependency | Structured | CRM lookups, LinkedIn data, company size databases |
| Relationship stakes | Low | Leads expect fast screening, not a relationship at this stage |
...
| Consistency | Variable (fatigue, bias, bad days) | Identical criteria applied every time |
...
For a rigorous way to run these numbers for your own situation, our [five-number ROI framework](/blog/ai-roi-calculator/) walks through the complete calculation.
...
| Factor | Score | Reasoning |
| --- | --- | --- |
| Repeatability | Low | Every account is different, every conversation is unique |
| Judgment complexity | High | Reading between the lines, navigating politics, strategic positioning |
| Data dependency | Mixed | Some CRM data, but mostly tacit knowledge and relationship context |
| Relationship stakes | Very high | These accounts are 60% of revenue |
...
| Factor | Score | Reasoning |
| --- | --- | --- |
| Repeatability | Split — 60% high, 40% low | The routine 60% follows clear patterns |
| Judgment complexity | Split | Routine is low-judgment; escalations require empathy and discretion |
| Data dependency | Structured for routine | Order tracking, return policies — all in existing systems |
| Relationship stakes | Split | Low for tracking updates, high for complaint resolution |

**Verdict: Branch 1 — Automate the 60% first, then evaluate hiring.**

Deploy an AI agent to handle the routine tickets. It [connects to your order management system](/lab/anatomy-of-ai-agent-tool-calling/), pulls real-time data, initiates returns, and provides accurate tracking updates around the clock. Clear escalation rules route complex issues to humans immediately.
...
| | Hire 5th Agent | Automate 60% + Keep 4 Agents |
| --- | --- | --- |
...
| Quality impact | Marginal — same process, more hands | Significant — humans focus on complex work only |
...
If these numbers feel abstract, the detailed [pricing breakdown](/blog/how-much-does-ai-chatbot-cost/) for different approaches — from off-the-shelf platforms to fully custom agents — shows exactly where the cost ranges land.
...
This is the [80/20 principle](/blog/why-businesses-fail-at-ai-agents/) applied to hiring: spend 80% of your effort designing the right split between human and machine work, and the 20% you spend on the technology will deliver disproportionate results.
...
An AI agent making decisions from a CRM with 30% duplicate contacts will make bad decisions 30% of the time — confidently and at scale. [Data readiness](/blog/ai-readiness-checklist/) isn't glamorous, but it's often the difference between automation that works and automation that creates more problems than it solves.
